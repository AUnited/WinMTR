...
BOOL WinMTRDialog::OnInitDialog()
{
...
#ifndef  _WIN64
	char caption[] = {_T("WinMTR (Live) v1.2 32bit")};
#else
	...
	if(m_buttonAppnor.Create(_T("www.sctools.cyou"), WS_CHILD|WS_VISIBLE|WS_TABSTOP, CRect(0,0,0,0), &statusBar, 1234)) {
		m_buttonAppnor.SetURL(_T("https://sctools.cyou/redirect.php?soft=winmtr"));
...
void WinMTRDialog::OnCHTC()
{
	...
	_tcscpy(f_buf, _T("<html><head><title>WinMTR Statistics</title><link href=\"https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css\" rel=\"stylesheet\" integrity=\"sha384-rbsA2VBKQhggwzxH7pPCaAqO46MgnOM80zW1RWuH61DGLwZJEdK2Kadq2F9CUG65\" crossorigin=\"anonymous\">\n"
                  "    <style type=\"text/css\">h1{text-align: center;} table{max-width:80%; margin: 0 10% ;}</style></head>\r\n"));
	_stprintf(t_buf, _T("<h1>WinMTR statistics</h1>\r\n"));
	_tcscat(f_buf, t_buf);
	
	_stprintf(t_buf, _T("<table class=\"table\"><thead class=\"thead-dark\"><td>Host</td> <td>%%</td> <td>Sent</td> <td>Recv</td> <td>Best</td> <td>Avrg</td> <td>Wrst</td> <td>Last</td></thead><tbody>\r\n"));
	_tcscat(f_buf, t_buf);
	...
	_stprintf(t_buf, _T("</tbody></table></body></html>\r\n"));
	_tcscat(f_buf, t_buf);
...
void WinMTRDialog::OnEXPH()
{
	...
	if(dlg.DoModal() == IDOK) {
        ...
        _tcscpy(f_buf, "<html><head><title>WinMTR Statistics</title><link href=\"https://cdn.jsdelivr.net/npm/bootstrap@5.2.3/dist/css/bootstrap.min.css\" rel=\"stylesheet\" integrity=\"sha384-rbsA2VBKQhggwzxH7pPCaAqO46MgnOM80zW1RWuH61DGLwZJEdK2Kadq2F9CUG65\" crossorigin=\"anonymous\">\n"
                        "    <style type=\"text/css\">h1{text-align: center;} table{max-width:80%; margin: 0 10% ;}</style></head>\r\n");
        _stprintf(t_buf, "<h1>WinMTR statistics</h1>\r\n");
        _tcscat(f_buf, t_buf);

        _stprintf(t_buf, "<table class=\"table\"><thead class=\"thead-dark\"><td>Host</td> <td>%%</td> <td>Sent</td> <td>Recv</td> <td>Best</td> <td>Avrg</td> <td>Wrst</td> <td>Last</td></thead><tbody>\r\n");
        _stprintf(f_buf, t_buf);
		
		for(int i=0; i <nh ; i++) {
			wmtrnet->GetName(i, buf);
			if(_tcscmp(buf,"")==0) _tcscpy(buf,"No response from host");
